        ...
        if(Revision == 2){              // Ejecuta la instrucción que recibe el PIC
            switch (smsR[0]){
                ...
                case 'r':               // Si el comando recibido es "r", llama a la función para ejecutar el barrido de detección de objetos
                    Radar();
                    break;
                case 'n':               // Si el comando recibido es "c", llama a la función para enviar nombre del robot
                    EnviarDatos('n');
                    break;
                default:                // Si el comando recibido está fuera de los listados, llama a la función para enviar "Error"
                    EnviarDatos('e');
                    break;
            }
            Revision = 0;
...
void Radar(void){
    ...
    while(Grados < 146){                //Realiza 146 lecturas en cada ultrasónico
        ...
        switch(blink){              //Condicional para el blink de los LED's durante la detección de objetos
            case 1:
                LATACLR = LED1;
                LATBCLR = LED2;
                LATBCLR = LED3;
